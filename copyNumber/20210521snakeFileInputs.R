...
# allIndices <- c(mouseIndices, mouseIndices2, humanIndice, humanIndice2, humanIndice3)
...
# mouseBeds$cnBed <- str_replace(mouseBeds$cnBed, "WG_IAD127899.20170720.designed.gc.bed", "WG_IAD127899.20170720.designed.forscript.GC.bed")
mouseBeds$cnBed <- str_replace(mouseBeds$cnBed, "CCP.gc.bed", "CCP.noTrack.GC.bed")
...
# mouseBeds$normals <- str_replace(mouseBeds$normals, "WG_IAD127899.20170720.designed.forscript.GC.bed",
#                                  "/mnt/DATA6/mouseData/normals/20191022_newNormals_noHyp.txt")
### id list
### converted original ID's into underscore - gets messy b/c Dan's perl script converts hyphens
### ideally I would recreate his perl script - no the hardest
...
# mouseBeds$normalsId <- str_replace(mouseBeds$normalsId, "WG_IAD127899.20170720.designed.forscript.GC.bed",
#                                    "/mnt/DATA6/mouseData/normals/20191022_newNormals_noHyp_IDlist.txt")



mouseBeds$segParam <- mouseBeds$cnBed
mouseBeds$segParam <- str_replace(mouseBeds$segParam, "IAD124056_167_Designed.del.nopool.gc.bed", paste(2, "mm10", sep = " "))
mouseBeds$segParam<- str_replace(mouseBeds$segParam, "IAD202670_167_Designed.gc.bed", paste(2, "mm10", sep = " "))
mouseBeds$segParam <- str_replace(mouseBeds$segParam, "20210726_OCAP_gc_noheader.bed", paste(2, "hg19", sep = " "))
mouseBeds$segParam <- str_replace(mouseBeds$segParam, "CCP.noTrack.GC.bed", paste(2, "hg19", sep = " "))
# mouseBeds$segParam <- str_replace(mouseBeds$segParam, "20210726_OCAP_gc_Andi2_noheader.bed", paste(c(2, "hg19"), sep = " "))
mouseBeds$segParam <- str_replace(mouseBeds$segParam, "20210726_OCAP_gc_Andi3_noheader.bed", paste(2, "hg19", sep = " "))
mouseBeds$segParam <- str_replace(mouseBeds$segParam, "IAD203665_173_Designed.gc.bed", paste(2, "hg19", sep = " "))
# mouseBeds$segParam <- str_replace(mouseBeds$segParam, "WG_IAD127899.20170720.designed.forscript.GC.bed", paste(2, "hg19", sep = " "))
